...
def get_rotary_pos_emb(cos, sin, position_ids):
...
class LlamaAttention(nn.Module):
    ...
    def forward(
        ...
        if past_key_value is not None:
            ...
            cached_value_states = value_states.view(bsz, q_len, self.num_heads, self.head_dim).transpose(1, 2)
            cached_key_states = key_states.view(bsz, q_len, self.num_heads, self.head_dim).transpose(1, 2)
...
class LlamaDecoderLayer(nn.Module):
    ...
    def forward(
        ...
        hidden_states = self.input_layernorm_groqensuref32(hidden_states)
        ...
        # Fully Connected
        residual = hidden_states
        hidden_states = self.post_attention_layernorm_groqensuref32(hidden_states)
...
class LlamaModel(LlamaPreTrainedModel):
    ...
    def forward(
        ...
        cos, sin = self.rotary_emb(past_key_values[0][0], seq_len=past_key_values_length)
        cos, sin = get_rotary_pos_emb(cos, sin, position_ids)
        ...
        hidden_states = self.norm_groqensuref32(hidden_states)
